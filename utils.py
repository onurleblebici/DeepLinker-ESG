...
def load_mxe_data(break_portion, path="./data/mxe/", dataset="ISELTA-ESG-full"):
    """Load mxe, generate training, validation and test set for link prediction task"""
    print('Loading {} dataset...'.format(dataset))
    
    #load graph
    idx_features_labels = np.genfromtxt("{}{}_nodes.txt".format(path, dataset), dtype=np.dtype(str))
    
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)
    labels = encode_onehot(idx_features_labels[:, -1])
    idx = np.array(idx_features_labels[:, 0], dtype=np.int32)
    idx_map = {j: i for i, j in enumerate(idx)}
    edges_unordered = np.genfromtxt("{}{}_edges.txt".format(path, dataset), dtype=np.int32)
