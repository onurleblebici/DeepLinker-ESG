...
class LogisticRegression(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        even_tensor = Variable(torch.LongTensor(range(node_num)[1::2]).cpu())
        odd_tensor = Variable(torch.LongTensor(range(node_num)[::2]).cpu())
...
#        features = torch.transpose(sum_features, 1,2).squeeze()
    ...
    def forward(self, input_head, input_tail):
        ...
        for k in range(self.K):
            ...
            if self.trainAttention == 'False':
                x,y =  alpha_matrix.size()
                #alpha_matrix = Variable((torch.ones(x,y)).cuda())            
                alpha_matrix = Variable((torch.ones(x,y)).cpu())            
